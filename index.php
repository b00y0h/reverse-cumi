...
if ($slideshow_select !== "none") {
	if ($slideshow_select == "flexslider") {
		get_template_part( 'loop', 'slideshow' );
	} elseif ($slideshow_select == "sequence") {
		get_template_part( 'loop', 'slideshow-sequence' );
	} elseif ($slideshow_select == "elastic") {	
		get_template_part( 'loop', 'slideshow-elastic' );
	} elseif ($slideshow_select == "iview") {	
		get_template_part( 'loop', 'slideshow-iview' );
	} elseif ($slideshow_select == "slit") {	
		get_template_part( 'loop', 'slideshow-slit' );
	} elseif ($slideshow_select == "jmpress") {	
		get_template_part( 'loop', 'slideshow-jmpress' );		
	} elseif ($slideshow_select == "multiitemslider") {	
		get_template_part( 'loop', 'slideshow-multiitemslider' );
	} elseif ($slideshow_select == "slideshow_plugin") {	
		$slideshow_plugin_shortcode = of_get_option('slideshow_plugin_shortcode');
		if ($slideshow_plugin_shortcode) {
			echo '<div class="container">';
			echo do_shortcode(' '.$slideshow_plugin_shortcode.' ');
			echo '</div>';
		}
	}
}
...
if ($layout_width == 'layout_width_1140') {
	if ($homepage_layout == 'with_sidebar')  { 
		$sh_columns = '4';
	} else {
		$sh_columns = '5';
	}	
...
foreach ($homepage_sorter as $key=>$value) {
    switch($key) {
        ...
        echo do_shortcode('[products columns="'.$sh_columns.'" ids="'.$products_ids.'" orderby="'.$products_by_id_orderby.'" order="'.$products_by_id_order.'"]');
